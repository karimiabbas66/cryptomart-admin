...
export class PersianCalendarService {
    ...
    PersianCalendar(gregorianDate: Date): string {
        ...
        if(Math.floor(this.month - 1)<10){
            return this.year + '-' + '0'+Math.floor(this.month - 1) + '-' + this.day;
        } else {
            return this.year + '-' + Math.floor(this.month - 1) + '-' + this.day;
        }


    }

    PersianCalendarNoDay(gregorianDate: Date): string {
        this.today = gregorianDate;
        let tem = new Date();
        if (tem.getFullYear() == this.today.getFullYear() &&
            tem.getMonth() == this.today.getMonth() && tem.getDate() == this.today.getDate()) {
            return 'امروز';
        } else {
            this.gregorianYear = this.today.getFullYear();
            this.gregorianMonth = this.today.getMonth() + 1;
    ...
    convertJalaliStringToEpoch(value: string): number {
        ...
        res = new Date(moment.from(value, 'en', 'YYYY-MM-DD').format('YYYY-MM-DD')).getTime();
        if (res < 0) {
            res = new Date(moment.from(value, 'fa', 'YYYY-MM-DD').format('YYYY-MM-DD')).getTime();
        }
        if (res && res.toString().length > 10) {
            offset = offset * 1000;
        }
    ...
    convertHijriStringToEpoch(value: string): number {
        ...
        res = new Date(moment.from(value, 'en', 'YYYY-MM-DD').format('YYYY-MM-DD')).getTime();
        if (res < 0) {
            res = new Date(moment.from(value, 'ar', 'YYYY-MM-DD').format('YYYY-MM-DD')).getTime();
        }
