...
export class BookEditorComponent implements OnInit, OnDestroy {
    ...
    private getSourceContent(pageNumber?: number) {
        ...
        this.bookService.getBookPageByPageNumber(this.bookResourceId, pageNumber ? pageNumber : this.pageNumber)
            .then((res: SourceContentModel) => {
                if (!res) {
                    this.toastr.error('فابل مورد نظر وجود ندارد');
                    this.router.navigate(['/sources/book-detail'], {queryParams: {bookId: this.bookResourceId}});
                } else {
    ...
    savePage() {
        ...
        this.showProgress=true;
        this.bookService.savePageInfo(this.sourceModel)
            .then(res => {
                this.showProgress=false;
    ...
    findById(id: any) {
        this.showProgress=true;
        this.bookService.getBookPageById(id)
            .then(res => {
                this.showProgress=false;
                this.sourceModel = res;
                this.editorModel = res.pageContent;
