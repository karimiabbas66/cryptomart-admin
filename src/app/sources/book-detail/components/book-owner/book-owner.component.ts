...
import {OrganizationListComponent} from '../../../../organization-management/organization-list/organization-list.component';
...
@Component({
    ...
    styleUrls: ['./book-owner.component.scss']
})
export class BookOwnerComponent implements OnInit, OnDestroy {
    ...
    public stepEnter() {
        ...
        if (this.book != null && this.book.id != null) {
            this.ownerService.getAllByBookId(this.book.id).then((res: OwnerModel[]) => {
                // this.selectedOwner = res.map((i) => { i.fullName = i.firstName + ' ' + i.lastName; return i; });
    ...
    afterUploadBookFile(event, i: number) {
